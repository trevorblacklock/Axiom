...
inline void data_from_nested_init_list_impl(
    ...
    for (const auto& x : list)
        if constexpr (N_ == 1)
            ...
            data_from_nested_init_list_impl<T_, N_ - 1>(x, ptr, shape, idx);
}
...
inline void shape_from_nested_init_list_impl(
    ...
    if constexpr (N_ >= 2)
        shape_from_nested_init_list_impl<T_, N_ - 1>(*list.begin(), shape,
                                                     size);
}

template<class T_, std::size_t N_>
inline void data_from_nested_init_list(const nested_init_list<T_, N_>& list,
                                       T_*                             ptr,
                                       const std::vector<std::size_t>& shape) {
...
template<class T_, std::size_t N_>
inline void shape_from_nested_init_list(const nested_init_list<T_, N_>& list,
                                        std::vector<std::size_t>&       shape,
                                        std::size_t&                    size) {
...
inline void verify_indices(const std::size_t* shape, It_ idx, Its_... idxs) {
    ...
    if constexpr (sizeof...(Its_) > 0)
        verify_indices(++shape, idxs...);
}

constexpr void transpose_helper(const std::size_t*              old_shape,
...
class ndarray {
 public:
    ...
    template<class Fn_, class Tp2_ = std::invoke_result_t<Fn_, Tp_>>
    ...
    constexpr auto transpose(const std::vector<std::size_t>& axes) const {
        ...
        for (auto& x : axes)
            ax_assert(x < rank(), "Axis cannot exceed rank of array!");
        ...
        detail::transpose_helper(old_shape, old_strides, new_shape.data(),
                                 new_strides.data(), 0, axes);
        auto new_extents = extent_type(new_shape, new_strides, size(), false);
    ...
    constexpr auto transpose() const {
        ...
        auto new_extents = extent_type(shape, strides, size(), false);
        auto array       = ndarray(data_, new_extents);
        return array;
    }
    ...
    constexpr auto view(Its_... idxs) const {
        ...
        auto  new_shape
            = std::vector(old_shape.begin() + sizeof...(Its_), old_shape.end());
        auto new_strides = std::vector(old_strides.begin() + sizeof...(Its_),
                                       old_strides.end());
        auto new_extents
            = extent_type(new_shape, new_strides, ranges::product(new_shape),
                          is_contiguous());
    ...
    ndarray(const ndarray<Tp_>& other) {
        *this = other;
    }

    ndarray(ndarray<Tp_>&& other) noexcept {
        *this = std::move(other);
    }
    ...
    ndarray(const Nl_<1>& data) {
        init_from_nl<1>(data);
    }

    ndarray(const Nl_<2>& data) {
        init_from_nl<2>(data);
    }

    ndarray(const Nl_<3>& data) {
        init_from_nl<3>(data);
    }

    ndarray(const Nl_<4>& data) {
        init_from_nl<4>(data);
    }

    ndarray(const Nl_<5>& data) {
        init_from_nl<5>(data);
    }
    ...
    constexpr auto& operator[](Its_... idxs) const {
        ax_assert(sizeof...(Its_) == rank(), "Incorrect number of indices!");
 ...
 private:
    ...
    constexpr void init_from_nl(const Nl_<N_>& data) {
        std::vector<std::size_t> shape;
        std::size_t              size;
        detail::shape_from_nested_init_list<data_type, N_>(data, shape, size);
        data_    = std::shared_ptr<Tp_[]>(new Tp_[size]);
        extents_ = std::make_unique<extent_type>(shape, size);
        detail::data_from_nested_init_list<data_type, N_>(data, data_.get(),
                                                          shape);
    }
