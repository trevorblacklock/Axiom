...
template<class Tp1_,
         ...
         class Fn_,
         class Tp3_ = std::invoke_result_t<Fn_, Tp1_, Tp2_>>
constexpr void broadcast_helper(const ndarray<Tp1_>& arr1,
...
template<class Tp1_, class Tp2_, class Fn_>
    requires(
        std::invocable<Fn_, Tp1_, Tp2_>
        && std::is_convertible_v<std::invoke_result_t<Fn_, Tp1_, Tp2_>, Tp2_>)
constexpr void
broadcast_scalar_self(const Tp1_& scalar, Tp2_& arr, Fn_&& func) {
    detail::broadcast_apply_scalar<false>(scalar, arr.data(), arr.data(), func,
                                          arr.size());
}
...
template<bool Sf_,
    ...
    requires(std::invocable<Fn_, Tp1_, Tp2_>)
constexpr auto
broadcast_scalar(const Tp1_& scalar, const ndarray<Tp2_>& arr1, Fn_&& func) {
    ndarray<Tp3_> arr2(arr1.extents());
    detail::broadcast_apply_scalar<Sf_>(scalar, arr1.data(), arr2.data(), func,
                                        arr2.size());
    return arr2;
}

template<class Tp1_, class Fn_>
    requires(std::invocable<Fn_, Tp1_, Tp1_>)
constexpr void
broadcast_self(ndarray<Tp1_>& arr1, const ndarray<Tp1_>& arr2, Fn_&& func) {
...
template<class Tp1_,
    ...
    requires(std::invocable<Fn_, Tp1_, Tp2_>)
constexpr auto
broadcast(const ndarray<Tp1_>& arr1, const ndarray<Tp2_>& arr2, Fn_&& func) {
